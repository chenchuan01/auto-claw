...
UITaskDetail.prototype.show = function(taskId) {
    ...
    ui.layout(
        ...
        '    <text id="btn_back" text="←" textSize="26sp" textColor="#FFFFFF" padding="4 4 12 4"/>' +
        '    <text text="任务详情" textSize="22sp" textColor="#FFFFFF" textStyle="bold" layout_weight="1"/>' +
        '    <text id="btn_edit" text="✎" textSize="24sp" textColor="#FFFFFF" padding="8 8 8 12"/>' +
        '  </horizontal>' +
        '  <!-- 内容区域 -->' +
        '  <scroll bg="' + C.bg + '">' +
        '    <vertical padding="20">' +
    ...
    function updateRunButton() {
        ...
        if (isRunning) {
            ui.btn_run_now.setText('■ 停止任务');
            ui.btn_run_now.attr('bg', C.error);
        } else {
            ui.btn_run_now.setText('▶ 执行任务');
